...
#### Question 3: The following question pertains to minority leadership and service. For the purpose of this question, minority is defined as women, people of color, and immigrants. Which of the following best describes your organization? (please provide your best estimate).?
...
Based on the metrics above, 89% of the survey respondents reported to at least of the minority service or leadership criteria. 
...
summary_table_eco_purchase<- data.frame(
  Measure = c("Mean", "Median", "Mode", "sd", "1st percentile", "3rd percentile"),
...
summary_table_eco_events<- data.frame(
  `Measure of events hosted in past year` = c("Mean", "Median", "Mode", "sd", "1st quantile", "3rd quantile"),
  Value = c(mean_value, median_value, mode_value, sd_value, q25_value, q75_value)
)
...
output_continuous%>% 
  ...
  htmlTable(header =  c("Mean","25th quantile", "Median", "Mode","75th quantile","Standard deviation"),
            rnames = c("on Purchasing or renting tools and equipment",
                       "on storage of tools and equipment",
                       "on maintenance of tools and equipment",
                       "on buying, storing, tracking and maintaining own tool+equiptment inventory in past year",
                       "# events that would have been impossible in past year",
                       "% of CBO's total events that would have been impossible in past year",
                       "# constituents that would have been negatively impacted in the past year "
            ),
            ...
            n.rgroup = c(4,3),
            caption = "Impact on CBO if TB did not exist",
            tfoot = "*in US dollars"
            )
...
#staff
#mean
round(mean(data$ci_staff, na.rm = T),1),
#25th
round(quantile(data$ci_staff, 0.25,na.rm=T),1),
#med
round(median(data$ci_staff, na.rm = T),1),
#75th
round(quantile(data$ci_staff, 0.75,na.rm=T),1),
#sd
round(sd(data$ci_staff, na.rm = T),1),

#projects
#mean
round(mean(data$ci_project, na.rm = T),1),
#25th
round(quantile(data$ci_project, 0.25,na.rm=T),1),
#med
round(median(data$ci_project, na.rm = T),1),
#75th
round(quantile(data$ci_project, 0.75,na.rm=T),1),
#sd
round(sd(data$ci_project, na.rm = T),1)
...
output_continuous%>% 
  addHtmlTableStyle(col.columns = c("none", "#F7F7F7"))%>% 
  htmlTable(header =  c("Mean","25th percentile","Median","75th percentile","Standard deviation"),
            rnames = c("Staff impacted",
                       "Projects impacted"
            ),
            # rgroup = c("Accomplishments through money saved through TB",
            #            "Where funding would be cut if TB did not exist"),
            #n.rgroup = c(3,3),
            caption = "Staff, projects that would be impacted without the TB"
            )
```
...
#
#%
round(c(nrow(data[data$ci_evimpact_qtt == 1,]),
        nrow(data[data$ci_evimpact_qtt == 1,])/nrow(data)*100,
        
       nrow(data[data$ci_evimpact_qlt == 1,]),
       nrow(data[data$ci_evimpact_qlt == 1,])/nrow(data)*100,
       
       nrow(data[data$ci_evimpact_dvt == 1,]),
       nrow(data[data$ci_evimpact_dvt == 1,])/nrow(data)*100,
       
       nrow(data[data$ci_evimpact_cst == 1,]),
       nrow(data[data$ci_evimpact_cst == 1,])/nrow(data)*100,
       
       nrow(data[!is.na(data$ci_evimpact_otr),]),
       nrow(data[!is.na(data$ci_evimpact_otr),])/nrow(data)*100
       ),1)),
...
output_continuous%>% 
  addHtmlTableStyle(col.columns = c("none", "#F7F7F7"))%>% 
  htmlTable(header =  c("Number of Respondants", "Percentage of Respondants (%)"),
            rnames = c("Quantity",
                                         "Quality",
                                         "Diversity",
                                         "Consistency",
            ...
            #            "Where funding would be cut if TB did not exist"),
            #n.rgroup = c(3,3),
            caption = "Ways community events would be impacted without the TB"
            )
...
output_continuous <- matrix(c(
  ...
  round(c(nrow(data[data$eve_type_fr == 1,]),
          nrow(data[data$eve_type_fr == 1,])/nrow(data)*100,
          
          nrow(data[data$eve_type_sp == 1,]),
          nrow(data[data$eve_type_sp == 1,])/nrow(data)*100,
          
          nrow(data[data$eve_type_ew == 1,]),
          nrow(data[data$eve_type_ew == 1,])/nrow(data)*100,
          
          nrow(data[data$eve_type_cb == 1,]),
          nrow(data[data$eve_type_cb == 1,])/nrow(data)*100,
          
          nrow(data[!is.na(data$eve_type_otr),]),
          nrow(data[!is.na(data$eve_type_otr),])/nrow(data)*100
          ),1)),
...
output_continuous%>% 
  ...
  htmlTable(header =  c("Number of Respondants", "Percentage of Respondants (%)"),
            ...
            #            "Where funding would be cut if TB did not exist"),
            #n.rgroup = c(3,3),
            caption = "Event Types TB helps to Complete"
            )
...
output_continuous <- matrix(c(
  ...
  round(c(mean(data$eve_fundraise_p, na.rm = T),
          quantile(data$eve_fundraise_p, 0.25,na.rm = T),
          median(data$eve_fundraise_p, na.rm = T),
          quantile(data$eve_fundraise_p, 0.75,na.rm = T),
          sd(data$eve_fundraise_p, na.rm = T),
          nrow(data[!is.na(data$eve_fundraise_p),])
          ),1)),
...
output_continuous%>% 
  addHtmlTableStyle(col.columns = c("none"))%>% 
  htmlTable(header =  c("Responses (in US dollars)"),
            rnames = c("Mean",
                       "25th percentile",
                       "Median",
                       "75th percentile",
            ...
            #            "Where funding would be cut if TB did not exist"),
            #n.rgroup = c(3,3),
            caption = "% of event expense budget saved by using TB equipment at event?"
            )
```
