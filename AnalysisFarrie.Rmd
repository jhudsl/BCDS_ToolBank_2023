...
gs4_auth()
```
```{r,out.width="50%",fig.show="hold",warning=FALSE,message=FALSE,echo=FALSE,include=FALSE}
...
#preserve the agree etc. reponses
data <- data%>%
  mutate(att_purtool_chr = att_purtool,
         att_strtool_chr = att_strtool,
         att_redtime_chr = att_redtime,
         att_evquality_chr = att_evquality,
         att_posimpact_chr = att_posimpact
           )

#create numeric equivlents
...
data%>%
  ...
  summarize(agree = sum(att_purtool_chr=="Agree",na.rm=T)/n(),
            strongly_agree = sum(att_purtool_chr=="Strongly agree",na.rm=T)/n(),
            neutral = sum(att_purtool_chr=="Neutral",na.rm=T)/n(),
            disagree = sum(att_purtool_chr=="Disagree",na.rm=T)/n(),
            strongly_disagree = sum(att_purtool_chr=="Strongly disagree",na.rm=T)/n(),
            N_A = sum(att_purtool_chr=="N/A",na.rm=T)/n())%>%
...
data%>%
  group_by(di_orgyearsworked)%>%
  summarize(agree = sum(att_strtool_chr=="Agree",na.rm=T)/n(),
            strongly_agree = sum(att_strtool_chr=="Strongly agree",na.rm=T)/n(),
            neutral = sum(att_strtool_chr=="Neutral",na.rm=T)/n(),
            disagree = sum(att_strtool_chr=="Disagree",na.rm=T)/n(),
            strongly_disagree = sum(att_strtool_chr=="Strongly disagree",na.rm=T)/n(),
            N_A = sum(att_strtool_chr=="N/A",na.rm=T)/n())%>%
  pivot_longer(agree:N_A, 
               names_to= "Response", 
               values_to = "Percentage")%>%
  mutate(Response = factor(Response, levels = c("strongly_agree",
...
data%>%
  group_by(di_orgyearsworked)%>%
  summarize(agree = sum(att_redtime_chr=="Agree",na.rm=T)/n(),
            strongly_agree = sum(att_redtime_chr=="Strongly agree",na.rm=T)/n(),
            neutral = sum(att_redtime_chr=="Neutral",na.rm=T)/n(),
            disagree = sum(att_redtime_chr=="Disagree",na.rm=T)/n(),
            strongly_disagree = sum(att_redtime_chr=="Strongly disagree",na.rm=T)/n(),
            N_A = sum(att_redtime_chr=="N/A",na.rm=T)/n())%>%
  pivot_longer(agree:N_A, 
               names_to= "Response", 
               values_to = "Percentage")%>%
  mutate(Response = factor(Response, levels = c("strongly_agree",
...
data%>%
  group_by(di_orgyearsworked)%>%
  summarize(agree = sum(att_evquality_chr=="Agree",na.rm=T)/n(),
            strongly_agree = sum(att_evquality_chr=="Strongly agree",na.rm=T)/n(),
            neutral = sum(att_evquality_chr=="Neutral",na.rm=T)/n(),
            disagree = sum(att_evquality_chr=="Disagree",na.rm=T)/n(),
            strongly_disagree = sum(att_evquality_chr=="Strongly disagree",na.rm=T)/n(),
            N_A = sum(att_evquality_chr=="N/A",na.rm=T)/n())%>%
  pivot_longer(agree:N_A, 
               names_to= "Response", 
               values_to = "Percentage")%>%
  mutate(Response = factor(Response, levels = c("strongly_agree",
...
data%>%
  group_by(di_orgyearsworked)%>%
  summarize(agree = sum(att_posimpact_chr=="Agree",na.rm=T)/n(),
            strongly_agree = sum(att_posimpact_chr=="Strongly agree",na.rm=T)/n(),
            neutral = sum(att_posimpact_chr=="Neutral",na.rm=T)/n(),
            disagree = sum(att_posimpact_chr=="Disagree",na.rm=T)/n(),
            strongly_disagree = sum(att_posimpact_chr=="Strongly disagree",na.rm=T)/n(),
            N_A = sum(att_posimpact_chr=="N/A",na.rm=T)/n())%>%
  pivot_longer(agree:N_A, 
               names_to= "Response", 
               values_to = "Percentage")%>%
  mutate(Response = factor(Response, levels = c("strongly_agree",
...
data%>%
  ...
  pivot_longer(cols=att_purtool_chr:att_posimpact_chr,names_to="question",values_to="response")%>%
  group_by(question)%>%
...
data%>%
  ...
  labs(x="Years Worked at CBO", 
       y="Percentage of respondants",
       title="Agreement of pos impact through TB services")
```
...
# Analysis surrounding questions 

Question: do organizations with higher savings through toolbank report more of a positive impact? 

```{r,out.width="50%",fig.show="hold",warning=FALSE,message=FALSE,echo=FALSE}
data%>%
  mutate(att_posimpact_chr = factor(att_posimpact_chr, levels = c("Strongly agree",
                                                "Agree",
                                                "Neutral", 
                                                "Disagree",
                                                "Strongly disagree", 
                                                 "N/A")))%>%    
  ggplot(aes(x=att_posimpact_chr, y=eco_totalcost))+
  geom_boxplot()+
  labs(x="Agreement with TB has positively impacted CBO",
       y= "Money org would spend on buying/storing/etc. equipment w/o TB?")

data%>%
  mutate(att_purtool_chr = factor(att_purtool_chr, levels = c("Strongly agree",
                                                "Agree",
                                                "Neutral", 
                                                "Disagree",
                                                "Strongly disagree", 
                                                 "N/A")))%>%  
  ggplot(aes(x=att_purtool_chr, y=eco_totalcost))+
  geom_boxplot()+
  labs(x="Agreement with TB reducing costs related to purchasing/renting tools/equipment",
       y= "Money org would spend on buying/storing/etc. equipment w/o TB?")

data%>%
  mutate(att_strtool_chr = factor(att_strtool_chr, levels = c("Strongly agree",
                                                "Agree",
                                                "Neutral", 
                                                "Disagree",
                                                "Strongly disagree", 
                                                 "N/A")))%>%  
  ggplot(aes(x=att_strtool_chr, y=eco_totalcost))+
  geom_boxplot()+
  labs(x="Agreement with TB reducing costs related to storing tools/equipment",
       y= "Money org would spend on buying/storing/etc. equipment w/o TB?")

data%>%
  mutate(att_redtime_chr = factor(att_redtime_chr, levels = c("Strongly agree",
                                                "Agree",
                                                "Neutral", 
                                                "Disagree",
                                                "Strongly disagree", 
                                                 "N/A")))%>%
  ggplot(aes(x=att_redtime_chr, y=eco_totalcost))+
  geom_boxplot()+
  labs(x="Agreement with TB reducing taff time for projects",
       y= "Money org would spend on buying/storing/etc. equipment w/o TB?")

data%>%
  mutate(att_evquality_chr = factor(att_evquality_chr, levels = c("Strongly agree",
                                                "Agree",
                                                "Neutral", 
                                                "Disagree",
                                                "Strongly disagree", 
                                                 "N/A")))%>%
  ggplot(aes(x=att_evquality_chr, y=eco_totalcost))+
  geom_boxplot()+
